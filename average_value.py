...
def PlotUpdate(new_data, x, y, ax, average, deviation, upper_limit, lower_limit, warning_upper, warning_lower, view_size):
    ...
    ax.plot([x[0], x[-1]], [lower_limit]*2, 'r--', linewidth=1,label='UEG')
    ax.plot([x[0], x[-1]], [warning_upper] * 2, 'y--', linewidth=0.5,label='OWG')
    ax.plot([x[0], x[-1]], [warning_lower] * 2, 'y--', linewidth=0.5,label='UWG')
